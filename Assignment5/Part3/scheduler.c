...
void spinlock_lock(AO_TS_t * lock) {
  ...
  while (AO_TS_SET== AO_test_and_set_acquire (lock)){   //;locked == AO_TS_SET){
...
#undef printf
void *safe_print(char *arg){
    static AO_TS_t printLock = AO_TS_INITIALIZER;
    spinlock_lock(&printLock);
    printf("%s",arg);
    spinlock_unlock(&printLock);
}
#define printf(arg) safe_print((char*)(arg))